...
Private Function ValidateSubjectLine(Subject As String) As validationResult
    ' Use Python backend for enhanced processing
    ValidateSubjectLine = ValidateSubjectWithPython(Subject)
End Function

' Original VBA validation function (kept as backup)
Private Function ValidateSubjectLineVBA(Subject As String) As validationResult
...
Public Function ProcessEmailWithPython(subject As String) As String
    ...
    cmd = "python """ & ThisWorkbook.Path & "\email_processor.py"" """ & EscapeForShell(subject) & """"
...
Public Function ValidateSubjectWithPython(subject As String) As validationResult
    ...
    ' Simple JSON parsing for validation result
    result.IsValid = InStr(jsonResult, """success"": true") > 0
    
    If result.IsValid Then
        ' Extract values from JSON (simplified parsing)
        result.RecordType = ExtractJSONValue(jsonResult, "record_type")
        result.CLLINumber = ExtractJSONValue(jsonResult, "clli_number")
        result.MSNumber = ExtractJSONValue(jsonResult, "ms_number")
        result.EventNumber = ExtractJSONValue(jsonResult, "event_number")
        result.RecordDate = ExtractJSONValue(jsonResult, "record_date")
        result.Description = ExtractJSONValue(jsonResult, "description")
        result.Status = ExtractJSONValue(jsonResult, "status")
        result.ErrorMessage = ""
    Else
        ' Extract error message
        result.ErrorMessage = ExtractJSONValue(jsonResult, "error")
        result.RecordType = ""
        result.CLLINumber = ""
        result.MSNumber = ""
        result.EventNumber = ""
...
'===============================================================================
' Python Question Sheet Console Integration
'===============================================================================

' Show Python Question Sheet GUI
Public Sub ShowPythonQuestionSheet()
    On Error GoTo ErrorHandler
    
    Dim cmd As String
    Dim shell As Object
    
    ' Build command to call Python GUI
    cmd = "python """ & ThisWorkbook.Path & "\question_sheet_gui.py"""
    
    ' Execute Python GUI (don't wait for completion)
    Set shell = CreateObject("WScript.Shell")
    shell.Run cmd, 1, False  ' 1 = Normal window, False = Don't wait
    
    Exit Sub
    
ErrorHandler:
    MsgBox "Error launching Python Question Sheet GUI: " & Err.Description, vbCritical, "GUI Error"
End Sub

' Show Python Question Sheet Console (alternative)
Public Sub ShowPythonQuestionSheetConsole()
    On Error GoTo ErrorHandler
    
    Dim cmd As String
    Dim shell As Object
    
    ' Build command to call Python console
    cmd = "python """ & ThisWorkbook.Path & "\question_sheet_console.py"""
    
    ' Execute Python console
    Set shell = CreateObject("WScript.Shell")
    shell.Run cmd, 1, True  ' 1 = Normal window, True = Wait for completion
    
    Exit Sub
    
ErrorHandler:
    MsgBox "Error launching Python Question Sheet Console: " & Err.Description, vbCritical, "Console Error"
End Sub

' Test Python Question Sheet Console
Public Sub TestPythonQuestionSheet()
    On Error GoTo ErrorHandler
    ...
    ' Test command (non-interactive)
    cmd = "python """ & ThisWorkbook.Path & "\question_sheet_console.py"" --test"
    
    Set shell = CreateObject("WScript.Shell")
    result = shell.Exec(cmd).StdOut.ReadAll
    
    MsgBox "Python Question Sheet Console Test:" & vbCrLf & result, vbInformation, "Console Test"
